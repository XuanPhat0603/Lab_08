...
bool BSTree::isEmpty() {
	return(m_root->m_left == NULL && m_root->m_right == NULL);
}
...
int BSTree::countNode() {
	return countNode(m_root);
}
int BSTree::countNode(TreeNode* n) {
	if (!n)
		return 0;
...
int BSTree::getHeight(TreeNode* n) {
	if (!n)
		return 0;
...
void BSTree::maxMinEachLevel() {
	...
	while (!q.empty()) {
		...
		cout << "Level: " << level++ << " => MIN: " << min << ", MAX " << max << "\n";
	}
}
